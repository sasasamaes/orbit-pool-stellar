...
export interface UseInvitationsReturn {
  ...
  createInvitation: (data: CreateInvitationRequest) => Promise<InvitationWithLinks>;
  sendEmailInvitation: (data: SendEmailInvitationRequest) => Promise<void>;
  createQuickInvitation: (groupId: string, expiresInHours?: number) => Promise<InvitationWithLinks>;
  revokeInvitation: (invitationId: string) => Promise<void>;
  copyInviteLink: (link: string) => Promise<void>;
  shareInvitation: (groupName: string, link: string, message?: string) => Promise<void>;
...
export function useInvitations(groupId?: string): UseInvitationsReturn {
  ...
  const loadInvitations = useCallback(async (targetGroupId: string) => {
    setIsLoading(true);
    setError(null);
  ...
  const loadAnalytics = useCallback(async (targetGroupId: string) => {
    try {
      const data = await InvitationService.getInvitationAnalytics(targetGroupId);
      setAnalytics(data);
    } catch (err: any) {
  ...
  const createInvitation = useCallback(async (data: CreateInvitationRequest): Promise<InvitationWithLinks> => {
  ...
  const sendEmailInvitation = useCallback(async (data: SendEmailInvitationRequest) => {
    setIsProcessing(true);
    setError(null);
  ...
  const createQuickInvitation = useCallback(async (
    ...
    try {
      const invitation = await InvitationService.createQuickInvitation(targetGroupId, expiresInHours);
  ...
  }, [groupId, currentGroupId, loadInvitations, loadAnalytics]);
...
export function usePublicInvitation(
  ...
  const acceptInvitation = useCallback(async (): Promise<{ group_id: string; membership_id: string }> => {
    if (!token && !code) {
