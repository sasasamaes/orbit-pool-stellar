...
router.post('/', requireAuth, asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { group_id, amount, stellar_transaction_id }: ContributionRequest = req.body;

  if (!group_id || !amount || !stellar_transaction_id) {
    throw createError('Group ID, amount, and Stellar transaction ID are required', 400);
  }
  ...
  const { error: balanceError } = await supabase
    ...
    .update({
      ...
      total_contributed: supabase.sql`total_contributed + ${amount}`
    })
    .eq('id', membership.id);
