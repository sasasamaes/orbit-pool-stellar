...
export class InvitationService {
  ...
  static async createInvitation(
    ...
    const { group_id, email, expires_in_hours = this.DEFAULT_EXPIRY_HOURS, message } = data;
    ...
    if (!membership || membership.role !== 'admin') {
      throw createError('Only group administrators can create invitations', 403);
    }
    ...
    if (email) {
      ...
      if (existingInvitation) {
        throw createError('Pending invitation already exists for this email', 400);
      }
    }
   ...
   */
  static async getInvitationByCode(code: string): Promise<InvitationData> {
    const { data: invitation, error } = await supabase
      .from('invitations')
      .select(`
        *,
  ...
  static async revokeInvitation(
    ...
    if (!membership || (membership.role !== 'admin' && invitation.inviter_id !== revoker_id)) {
